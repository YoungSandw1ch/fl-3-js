...
})({"kFn5I":[function(require,module,exports) {
"use strict";
...
module.bundle.HMR_BUNDLE_ID = "59171e9911073073";
...
},{}],"lDHsX":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
var _promisesCss = require("../css/promises.css");
...
const horses = Array.from(document.querySelectorAll('input[name="horse"]')).map((el)=>el.id);
let raceCounter = 0;
let horseName = null;
...
function findAndRenderWinner(promises) {
    Promise.race(promises).then(({ horse , time  })=>{
        ...
        const tableMarkup = createTableBodyMarkup({
            horse,
            time,
            raceCounter
        });
...
},{"../css/promises.css":"kvwMA","bootstrap.native":"6udud","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"kvwMA":[function() {},{}],"6udud":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "default", ()=>BSN);
/*!
  * Native JavaScript for Bootstrap v4.2.0 (https://thednp.github.io/bootstrap.native/)
  * Copyright 2015-2022 © dnp_theme
  * Licensed under MIT (https://github.com/thednp/bootstrap.native/blob/master/LICENSE)
  */ /** @type {Record<string, any>} */ const EventRegistry = {};
/**
 ...
 */ function globalListener(e) {
    ...
    [
        ...
        /* istanbul ignore else */ if (element === that) [
            ...listenersMap
        ].forEach((listenerMap)=>{
            ...
            if (options && options.once) removeListener(element, type, listener, options);
        });
    });
}
/**
 ...
 */ const addListener = (element, eventType, listener, options)=>{
    ...
    if (!EventRegistry[eventType]) EventRegistry[eventType] = new Map();
    const oneEventMap = EventRegistry[eventType];
    if (!oneEventMap.has(element)) oneEventMap.set(element, new Map());
 ...
 */ const removeListener = (element, eventType, listener, options)=>{
    ...
    const { options: eventOptions  } = savedOptions !== undefined ? savedOptions : {
        options
    };
 ...
 */ function emulateTransitionEnd(element, handler) {
     ...
     */ const transitionEndWrapper = (e)=>{
            /* istanbul ignore else */ if (e.target === element) {
        ...
        setTimeout(()=>{
            /* istanbul ignore next */ if (!called) dispatchEvent(element, endEvent);
        }, duration + delay + 17);
    } else handler.apply(element, [
        endEvent
    ]);
}
/**
...
/** @type {Map<string, Map<HTMLElement, Record<string, any>>>} */ const componentData = new Map();
/**
 ...
 */ const Data = {
   ...
   */ set: (element, component, instance)=>{
        ...
        /* istanbul ignore else */ if (!componentData.has(component)) componentData.set(component, new Map());
   ...
   */ remove: (element, component)=>{
        ...
        /* istanbul ignore else */ if (instanceMap.size === 0) componentData.delete(component);
    }
};
/**
 ...
 */ function OriginalEvent(EventType, config) {
    ...
    /* istanbul ignore else */ if (isObject(config)) ObjectAssign(OriginalCustomEvent, config);
...
/** @type {string} */ const alertString = "alert";
 ...
 */ function normalizeValue(value) {
    if ([
        "true",
        true
    ].includes(value)) // if ('true' === value) { // boolean
    return true;
    if ([
        "false",
        false
    ].includes(value)) // if ('false' === value) { // boolean
    return false;
 ...
 */ function normalizeOptions(element, defaultOps, inputOps, ns) {
    ...
    /** @type {Record<string, any>} */ const normalOps = {};
    /** @type {Record<string, any>} */ const dataOps = {};
    const title = "title";
    ...
    ObjectKeys(defaultOps).forEach((k)=>{
        /* istanbul ignore else */ if (k in inputOps) normalOps[k] = inputOps[k];
        else if (k in dataOps) normalOps[k] = dataOps[k];
        else normalOps[k] = k === title ? getAttribute(element, title) : defaultOps[k];
    });
    return normalOps;
}
var version = "4.2.0";
...
----------------------------------------------------- */ /** Returns a new `BaseComponent` instance. */ class BaseComponent {
    /**
   ...
   */ constructor(target, config){
        ...
        if (!element) throw Error(`${self.name} Error: "${target}" is not a valid selector.`);
        /** @static @type {BSN.ComponentOptions} */ self.options = {};
        const prevInstance = Data.get(element, self.name);
        if (prevInstance) prevInstance.dispose();
        /** @type {HTMLElement} */ self.element = element;
        /* istanbul ignore else */ if (self.defaults && ObjectKeys(self.defaults).length) self.options = normalizeOptions(element, self.defaults, config || {}, "bs");
        Data.set(element, self.name, self);
    }
    /* eslint-disable */ /* istanbul ignore next */ /** @static */ get version() {
        return Version;
    }
    /* eslint-enable */ /* istanbul ignore next */ /** @static */ get name() {
        return this.constructor.name;
    }
    /* istanbul ignore next */ /** @static */ get defaults() {
 ...
 */ function toggleAlertHandler(self, add) {
    ...
    const { dismiss  } = self;
    /* istanbul ignore else */ if (dismiss) action(dismiss, mouseclickEvent, self.close);
}
// ALERT DEFINITION
// ================
/** Creates a new Alert instance. */ class Alert extends BaseComponent {
    /** @param {HTMLElement | string} target element or selector */ constructor(target){
        super(target);
        // bind
        const self = this;
        ...
        const { element  } = self;
        // the dismiss button
        /** @static @type {HTMLElement?} */ self.dismiss = querySelector(alertDismissSelector, element);
        // add event listener
   ...
   */ get name() {
    ...
    /* eslint-enable */ // ALERT PUBLIC METHODS
    // ====================
    /**
   ...
   */ close(e) {
        ...
        const { element  } = self;
        /* istanbul ignore else */ if (element && hasClass(element, showClass)) {
            dispatchEvent(element, closeAlertEvent);
            if (closeAlertEvent.defaultPrevented) return;
            removeClass(element, showClass);
            if (hasClass(element, fadeClass)) emulateTransitionEnd(element, ()=>alertTransitionEnd(self));
            else alertTransitionEnd(self);
        }
    }
    /** Remove the component from target element. */ dispose() {
...
/** @type {string} */ const buttonString = "button";
/** @type {string} */ const buttonComponent = "Button";
...
// BUTTON DEFINITION
// =================
/** Creates a new `Button` instance. */ class Button extends BaseComponent {
    /**
   ...
   */ constructor(target){
        ...
        const { element  } = self;
        // set initial state
        /** @type {boolean} */ self.isActive = hasClass(element, activeClass);
   ...
   */ get name() {
    ...
    /* eslint-enable */ // BUTTON PUBLIC METHODS
    // =====================
    /**
   ...
   */ toggle(e) {
    ...
    /** Removes the `Button` component from the target element. */ dispose() {
...
/** @type {Map<HTMLElement, any>} */ const TimeCache = new Map();
/**
 ...
 */ const Timer = {
   ...
   */ set: (element, callback, delay, key)=>{
        if (!isHTMLElement(element)) return;
        /* istanbul ignore else */ if (key && key.length) {
            /* istanbul ignore else */ if (!TimeCache.has(element)) TimeCache.set(element, new Map());
   ...
   */ get: (element, key)=>{
        ...
        const keyTimers = TimeCache.get(element);
        if (key && key.length && keyTimers && keyTimers.get) return keyTimers.get(key) || /* istanbul ignore next */ null;
   ...
   */ clear: (element, key)=>{
        ...
        if (key && key.length) {
            const keyTimers = TimeCache.get(element);
            /* istanbul ignore else */ if (keyTimers && keyTimers.get) {
                clearTimeout(keyTimers.get(key));
                keyTimers.delete(key);
                /* istanbul ignore else */ if (keyTimers.size === 0) TimeCache.delete(element);
            }
        } else {
...
/** @type {string} */ const carouselString = "carousel";
/** @type {string} */ const carouselComponent = "Carousel";
/**
 ...
 */ function getTargetElement(element) {
    ...
    return targetAttr.map((att)=>{
        ...
        if (attValue) return att === dataBsParent ? closest(element, attValue) : querySelector(attValue, doc);
        return null;
    }).filter((x)=>x)[0];
}
 ...
 */ function carouselTransitionEndHandler(self) {
    ...
    /* istanbul ignore else */ if (self.isAnimating && getCarouselInstance(element)) {
        ...
        if (!getDocument(element).hidden && options.interval && !self.isPaused) self.cycle();
    }
}
/**
 ...
 */ function carouselPauseHandler() {
    ...
    /* istanbul ignore else */ if (self && !self.isPaused && !Timer.get(element, pausedClass)) addClass(element, pausedClass);
}
/**
 ...
 */ function carouselResumeHandler() {
    const element = this;
    const self = getCarouselInstance(element);
    /* istanbul ignore else */ if (self && self.isPaused && !Timer.get(element, pausedClass)) self.cycle();
}
/**
 ...
 */ function carouselIndicatorHandler(e) {
    ...
    if (indicator && !hasClass(indicator, activeClass) // event target is not active
     && !Number.isNaN(newIndex)) self.to(newIndex); // do the slide
}
/**
 ...
 */ function carouselControlsHandler(e) {
    ...
    /* istanbul ignore else */ if (orientation === "next") self.next();
 ...
 */ function carouselKeyHandler({ code , target  }) {
    ...
    const [element] = [
        ...querySelectorAll(carouselSelector, doc)
    ].filter((x)=>isElementInScrollRange(x));
    const self = getCarouselInstance(element);
    /* istanbul ignore next */ if (!self || self.isAnimating || /textarea|input/i.test(target.tagName)) return;
    const RTL = isRTL(element);
    const arrowKeyNext = !RTL ? keyArrowRight : keyArrowLeft;
    const arrowKeyPrev = !RTL ? keyArrowLeft : keyArrowRight;
    /* istanbul ignore else */ if (code === arrowKeyPrev) self.prev();
 ...
 */ function carouselPointerDownHandler(e) {
    ...
    if ([
        ...controls,
        ...indicators
    ].some((el)=>el === target || el.contains(target))) return;
    if (!self || self.isAnimating || self.isTouch) return;
    startX = e.pageX;
    /* istanbul ignore else */ if (element.contains(target)) {
        self.isTouch = true;
 ...
 */ function carouselPointerUpHandler(e) {
    ...
    /* istanbul ignore next */ if (!self) return;
    ...
    /* istanbul ignore else */ if (!element.contains(target) || Math.abs(startX - endX) < 120) {
        // reset pointer position
        startX = 0;
        currentX = 0;
        endX = 0;
    ...
    /* istanbul ignore else */ if (currentX < startX) self.to(index + (RTL ? -1 : 1));
 ...
 */ function activateCarouselIndicator(self, pageIndex) {
    ...
    [
        ...indicators
    ].forEach((x)=>removeClass(x, activeClass));
    /* istanbul ignore else */ if (self.indicators[pageIndex]) addClass(indicators[pageIndex], activeClass);
}
/**
 ...
 */ function toggleCarouselHandlers(self, add) {
    ...
    if (touch && slides.length > 2) action(element, pointerdownEvent, carouselPointerDownHandler, passiveHandler);
    /* istanbul ignore else */ if (controls.length) controls.forEach((arrow)=>{
        /* istanbul ignore else */ if (arrow) action(arrow, mouseclickEvent, carouselControlsHandler);
    });
    /* istanbul ignore else */ if (indicators.length) indicators.forEach((indicator)=>{
...
// CAROUSEL DEFINITION
// ===================
/** Creates a new `Carousel` instance. */ class Carousel extends BaseComponent {
    /**
   ...
   */ constructor(target, config){
        ...
        /** @type {string} */ self.direction = isRTL(element) ? "right" : "left";
        /** @type {number} */ self.index = 0;
        /** @type {boolean} */ self.isTouch = false;
        // carousel elements
        ...
        /** @type {HTMLElement?} */ self.indicator = querySelector(`.${carouselString}-indicators`, element);
        // a LIVE collection is prefferable
        /** @type {HTMLElement[]} */ self.indicators = [
        ...
        self.options.interval = options.interval === true ? carouselDefaults.interval : options.interval;
        // set first slide active if none
        /* istanbul ignore else */ if (getActiveIndex(self) < 0) {
            addClass(slides[0], activeClass);
            /* istanbul ignore else */ if (self.indicators.length) activateCarouselIndicator(self, 0);
        }
  ...
  */ get isAnimating() {
    ...
    // =======================
    /** Slide automatically through items. */ cycle() {
        const self = this;
        const { element , options , isPaused , index ,  } = self;
        ...
        Timer.set(element, ()=>{
            ...
            /* istanbul ignore else */ if (self.element && !self.isPaused && !self.isTouch && isElementInScrollRange(element)) self.to(index + 1);
        }, options.interval, carouselString);
    }
    /** Pause the automatic cycle. */ pause() {
        const self = this;
        const { element , options  } = self;
        /* istanbul ignore else */ if (!self.isPaused && options.interval) {
            addClass(element, pausedClass);
            Timer.set(element, ()=>{}, 1, pausedClass);
        }
    }
    /** Slide to the next item. */ next() {
        const self = this;
        /* istanbul ignore else */ if (!self.isAnimating) self.to(self.index + 1);
    }
    /** Slide to the previous item. */ prev() {
        const self = this;
        /* istanbul ignore else */ if (!self.isAnimating) self.to(self.index - 1);
    }
    /**
   ...
   */ to(idx) {
        const self = this;
        const { element , slides , options ,  } = self;
        const activeItem = getActiveIndex(self);
        const RTL = isRTL(element);
        let next = idx;
        ...
        /* istanbul ignore else */ if (activeItem < next || activeItem === 0 && next === slides.length - 1) self.direction = RTL ? "right" : "left"; // next
        else if (activeItem > next || activeItem === slides.length - 1 && next === 0) self.direction = RTL ? "left" : "right"; // prev
        ...
        if (getElementTransitionDuration(slides[next]) && hasClass(element, "slide")) Timer.set(element, ()=>{
            ...
            Timer.set(element, ()=>{
                Timer.clear(element, dataBsSlide);
                // check for element, might have been disposed
                /* istanbul ignore else */ if (element && options.interval && !self.isPaused) self.cycle();
                dispatchEvent(element, carouselSlidEvent);
            }, 0, dataBsSlide);
        }
    }
    /** Remove `Carousel` component from target. */ dispose() {
        const self = this;
        const { slides  } = self;
        const itemClasses = [
            "start",
...
/** @type {string} */ const collapseString = "collapse";
/** @type {string} */ const collapseComponent = "Collapse";
 ...
 */ function collapseContent(self) {
    ...
    emulateTransitionEnd(element, ()=>{
        Timer.clear(element);
        /* istanbul ignore else */ if (parent) Timer.clear(parent);
 ...
 */ function toggleCollapseHandler(self, add) {
    const action = add ? addListener : removeListener;
    const { triggers  } = self;
    /* istanbul ignore else */ if (triggers.length) triggers.forEach((btn)=>action(btn, mouseclickEvent, collapseClickHandler));
}
 ...
 */ function collapseClickHandler(e) {
    ...
    /* istanbul ignore else */ if (self) self.toggle();
    // event target is anchor link #398
    if (trigger && trigger.tagName === "A") e.preventDefault();
}
// COLLAPSE DEFINITION
// ===================
/** Returns a new `Colapse` instance. */ class Collapse extends BaseComponent {
    /**
   ...
   */ constructor(target, config){
        ...
        /** @type {HTMLElement[]} */ self.triggers = [
            ...querySelectorAll(collapseToggleSelector, doc)
        ].filter((btn)=>getTargetElement(btn) === element);
        // set parent accordion
        /** @type {HTMLElement?} */ self.parent = querySelector(options.parent, doc) || getTargetElement(element) || null;
        // add event listeners
   ...
   */ get defaults() {
    ...
    /* eslint-enable */ // COLLAPSE PUBLIC METHODS
    // =======================
    /** Toggles the visibility of the collapse. */ toggle() {
        const self = this;
        if (!hasClass(self.element, showClass)) self.show();
        else self.hide();
    }
    /** Hides the collapse. */ hide() {
        const self = this;
        const { triggers , element  } = self;
        if (Timer.get(element)) return;
        collapseContent(self);
        /* istanbul ignore else */ if (triggers.length) triggers.forEach((btn)=>addClass(btn, `${collapseString}d`));
    }
    /** Shows the collapse. */ show() {
        const self = this;
        const { element , parent , triggers ,  } = self;
        let activeCollapse;
        ...
        if ((!parent || !Timer.get(parent)) && !Timer.get(element)) {
            ...
            expandCollapse(self);
            /* istanbul ignore else */ if (triggers.length) triggers.forEach((btn)=>removeClass(btn, `${collapseString}d`));
        }
    }
    /** Remove the `Collapse` component from the target `Element`. */ dispose() {
        const self = this;
...
/** @type {string} */ const dropdownComponent = "Dropdown";
/**
 ...
 */ function isEmptyAnchor(element) {
    ...
    return isHTMLElement(element) && (hasAttribute(element, "href") && element.href.slice(-1) === "#" || parentAnchor && hasAttribute(parentAnchor, "href") && parentAnchor.href.slice(-1) === "#");
}
 ...
 */ function styleDropdown(self) {
    ...
    /* istanbul ignore next: this test requires a navbar */ if (getElementStyle(menu, "position") === "static") return;
    const RTL = isRTL(element);
    ...
    let positionClass = dropdownMenuClasses.find((c)=>hasClass(parentElement, c)) || /* istanbul ignore next: fallback position */ dropdownString;
    /** @type {Record<string, Record<string, any>>} */ let dropdownMargin = {
        dropdown: [
            offset,
            0,
            0
    ...
    /** @type {Record<string, Record<string, any>>} */ const dropdownPosition = {
        dropdown: {
            top: "100%"
        },
        dropup: {
    ...
    if (verticalClass.includes(positionClass) && menuEnd) /* istanbul ignore else */ {
        if (menuEnd) {
            const endAdjust = !RTL && leftExceed || RTL && rightExceed ? "menuStart" : /* istanbul ignore next */ "menuEnd";
 ...
 */ function toggleDropdownDismiss(self) {
    ...
    /* istanbul ignore else */ if (options.display === "dynamic") [
        scrollEvent,
        resizeEvent
    ].forEach((ev)=>{
 ...
 */ function getCurrentOpenDropdown(element) {
    ...
    if (currentParent && currentParent.length) return [
 ...
 */ function dropdownDismissHandler(e) {
    const { target , type  } = e;
    /* istanbul ignore next: impossible to satisfy */ if (!target || !target.closest) return; // some weird FF bug #409
    ...
    if (type === mouseclickEvent && isEmptyAnchor(target)) e.preventDefault();
    if (type === focusEvent && (target === element || target === menu || menu.contains(target))) return;
    /* istanbul ignore else */ if (isForm || hasData) ;
 ...
 */ function dropdownClickHandler(e) {
    ...
    const { target  } = e;
    const self = getDropdownInstance(element);
    /* istanbul ignore else */ if (self) {
        self.toggle();
        /* istanbul ignore else */ if (target && isEmptyAnchor(target)) e.preventDefault();
    }
}
/**
 ...
 */ function dropdownKeyHandler(e) {
    ...
    /* istanbul ignore next: impossible to satisfy */ if (!self || !activeElement) return;
    ...
    if (menuItems && menuItems.length && [
        ...
        /* istanbul ignore else */ if (activeElement === element) idx = 0;
        else if (code === keyArrowUp) idx = idx > 1 ? idx - 1 : 0;
        else if (code === keyArrowDown) idx = idx < menuItems.length - 1 ? idx + 1 : idx;
        /* istanbul ignore else */ if (menuItems[idx]) focus(menuItems[idx]);
    }
 ...
 */ function dropdownLayoutHandler() {
    const element = getCurrentOpenDropdown(this);
    const self = element && getDropdownInstance(element);
    /* istanbul ignore else */ if (self && self.open) styleDropdown(self);
}
// DROPDOWN DEFINITION
// ===================
/** Returns a new Dropdown instance. */ class Dropdown extends BaseComponent {
    /**
   ...
   */ constructor(target, config){
        ...
        // set targets
        /** @type {(Element | HTMLElement)} */ self.parentElement = parentElement;
        /** @type {(Element | HTMLElement)} */ self.menu = querySelector(`.${dropdownMenuClass}`, parentElement);
        // set initial state to closed
        /** @type {boolean} */ self.open = false;
        // add event listener
   ...
   */ get defaults() {
    ...
    /* eslint-enable */ // DROPDOWN PUBLIC METHODS
    // =======================
    /** Shows/hides the dropdown menu to the user. */ toggle() {
        const self = this;
        if (self.open) self.hide();
        else self.show();
    }
    /** Shows the dropdown menu to the user. */ show() {
        const self = this;
        const { element , open , menu , parentElement ,  } = self;
        /* istanbul ignore next */ if (open) return;
    ...
    /** Hides the dropdown menu from the user. */ hide() {
        const self = this;
        const { element , open , menu , parentElement ,  } = self;
        /* istanbul ignore next */ if (!open) return;
        [
    ...
    /** Removes the `Dropdown` component from the target element. */ dispose() {
        const self = this;
        if (self.open) self.hide();
...
/** @type {string} */ const modalString = "modal";
/** @type {string} */ const modalComponent = "Modal";
/**
 ...
 */ function getParentNode(node) {
    ...
    return node.assignedSlot // step into the shadow DOM of the parent of a slotted node
 ...
 */ const isTableElement = (element)=>element && [
        ...
        "TD",
        "TH"
    ].includes(element.tagName) || false;
/**
 ...
 */ function getElementContainer(element, getOffset) {
    const majorBlockTags = [
        "HTML",
        "BODY"
    ];
    if (getOffset) {
        /** @type {any} */ let { offsetParent  } = element;
        const win = getWindow(element);
        while(offsetParent && (isTableElement(offsetParent) || isHTMLElement(offsetParent) && ![
            "sticky",
            "fixed"
        ].includes(getElementStyle(offsetParent, "position"))))offsetParent = offsetParent.offsetParent;
        if (!offsetParent || majorBlockTags.includes(offsetParent.tagName) || getElementStyle(offsetParent, "position") === "static") offsetParent = win;
        return offsetParent;
    }
    /** @type {ParentNode[]} */ const containers = [];
    /** @type {ParentNode} */ let { parentNode  } = element;
    while(parentNode && !majorBlockTags.includes(parentNode.nodeName)){
        parentNode = getParentNode(parentNode);
        /* istanbul ignore else */ if (!(isShadowRoot(parentNode) || !!parentNode.shadowRoot || isTableElement(parentNode))) containers.push(parentNode);
    }
    return containers.find((c, i)=>{
        if (getElementStyle(c, "position") !== "relative" && containers.slice(i + 1).every((r)=>getElementStyle(r, "position") === "static")) return c;
        return null;
...
/** @param {(HTMLElement | Document)=} parent */ const getFixedItems = (parent)=>[
 ...
 */ function setScrollbar(element, overflow) {
    ...
    const fixedItems = getFixedItems(bd);
    /* istanbul ignore else */ if (overflow) {
        setElementStyle(bd, {
            overflow: "hidden",
            paddingRight: `${bodyPad + sbWidth}px`
        });
        /* istanbul ignore else */ if (fixedItems.length) fixedItems.forEach((fixed)=>{
            const itemPadValue = getElementStyle(fixed, "paddingRight");
            fixed.style.paddingRight = `${parseInt(itemPadValue, 10) + sbWidth}px`;
            /* istanbul ignore else */ if ([
                stickyTopClass,
                positionStickyClass
            ].some((c)=>hasClass(fixed, c))) {
 ...
 */ function createElement(param) {
    ...
    if (typeof param === "string") return getDocument().createElement(param);
    const { tagName  } = param;
    const attr = {
        ...param
    };
...
/** @type {string} */ const offcanvasString = "offcanvas";
 ...
 */ function setModalScrollbar(self) {
    ...
    /* istanbul ignore else */ if (!modalOverflow && scrollbarWidth) {
 ...
 */ function toggleModalHandler(self, add) {
    const action = add ? addListener : removeListener;
    const { triggers  } = self;
    /* istanbul ignore else */ if (triggers.length) triggers.forEach((btn)=>action(btn, mouseclickEvent, modalClickHandler));
}
/**
 ...
 */ function afterModalHide(self, callback) {
    ...
    /* istanbul ignore else */ if (focusElement) focus(focusElement);
    /* istanbul ignore else */ if (callback) callback();
 ...
 */ function beforeModalShow(self) {
    ...
    /* istanbul ignore else */ if (!getCurrentOpen(element)) setElementStyle(getDocumentBody(element), {
        overflow: "hidden"
    });
    addClass(element, showClass);
 ...
 */ function beforeModalHide(self, callback) {
    ...
    } else afterModalHide(self, callback);
}
 ...
 */ function modalClickHandler(e) {
    ...
    /* istanbul ignore else */ if (trigger && trigger.tagName === "A") e.preventDefault();
 ...
 */ function modalKeyHandler({ code , target  }) {
    ...
    const self = element && getModalInstance(element);
    const { options  } = self;
    /* istanbul ignore else */ if (options.keyboard && code === keyEscape // the keyboard option is enabled and the key is 27
 ...
 */ function modalDismissHandler(e) {
    ...
    /* istanbul ignore next: must have a filter */ if (!self || Timer.get(element)) return;
    ...
    /* istanbul ignore else */ if (isStatic && !targetInsideDialog) Timer.set(element, ()=>{
...
// MODAL DEFINITION
// ================
/** Returns a new `Modal` instance. */ class Modal extends BaseComponent {
    /**
   ...
   */ constructor(target, config){
        ...
        const { element  } = self;
        // the modal-dialog
        /** @type {(HTMLElement)} */ self.modalDialog = querySelector(`.${modalString}-dialog`, element);
        // modal can have multiple triggering elements
        /** @type {HTMLElement[]} */ self.triggers = [
            ...querySelectorAll(modalToggleSelector, getDocument(element))
        ].filter((btn)=>getTargetElement(btn) === element);
        // additional internals
        /** @type {boolean} */ self.isStatic = self.options.backdrop === "static";
        /** @type {boolean} */ self.hasFade = hasClass(element, fadeClass);
        /** @type {HTMLElement?} */ self.relatedTarget = null;
        /** @type {HTMLBodyElement | HTMLElement} */ self.container = getElementContainer(element);
   ...
   */ get defaults() {
    ...
    /* eslint-enable */ // MODAL PUBLIC METHODS
    // ====================
    /** Toggles the visibility of the modal. */ toggle() {
        const self = this;
        if (hasClass(self.element, showClass)) self.hide();
        else self.show();
    }
    /** Shows the modal to the user. */ show() {
        const self = this;
        const { element , options , hasFade , relatedTarget , container ,  } = self;
        const { backdrop  } = options;
        let overlayDelay = 0;
        ...
        if (currentOpen && currentOpen !== element) {
            ...
            const that1 = this1 || /* istanbul ignore next */ getInstance(currentOpen, "Offcanvas");
            that1.hide();
        }
        if (backdrop) {
            if (!container.contains(overlay)) appendOverlay(container, hasFade, true);
            ...
            /* istanbul ignore else */ if (currentOpen && hasClass(overlay, showClass)) hideOverlay();
        }
    }
    /**
   ...
   */ hide(callback) {
        const self = this;
        const { element , hasFade , relatedTarget ,  } = self;
        if (!hasClass(element, showClass)) return;
        ...
        // if (hasFade && callback) {
        /* istanbul ignore else */ if (hasFade) emulateTransitionEnd(element, ()=>beforeModalHide(self, callback));
        else beforeModalHide(self, callback);
    }
    /**
   ...
   */ update() {
        const self = this;
        /* istanbul ignore else */ if (hasClass(self.element, showClass)) setModalScrollbar(self);
    }
    /** Removes the `Modal` component from target element. */ dispose() {
        const self = this;
...
/** @type {string} */ const offcanvasComponent = "Offcanvas";
 ...
 */ function beforeOffcanvasShow(self) {
    const { element , options  } = self;
    /* istanbul ignore else */ if (!options.scroll) {
 ...
 */ function offcanvasDismissHandler(e) {
    ...
    /* istanbul ignore next: must have a filter */ if (!self) return;
    const { options , triggers  } = self;
    const { backdrop  } = options;
    const trigger = closest(target, offcanvasToggleSelector);
    const selection = getDocument(element).getSelection();
    if (overlay.contains(target) && backdrop === "static") return;
    /* istanbul ignore else */ if (!(selection && selection.toString().length) && (!element.contains(target) && backdrop && (!trigger || triggers.includes(target)) || offCanvasDismiss && offCanvasDismiss.contains(target))) {
        self.relatedTarget = offCanvasDismiss && offCanvasDismiss.contains(target) ? offCanvasDismiss : null;
        self.hide();
    }
    /* istanbul ignore next */ if (trigger && trigger.tagName === "A") e.preventDefault();
}
/**
 ...
 */ function offcanvasKeyDismissHandler({ code , target  }) {
    const element = querySelector(offcanvasActiveSelector, getDocument(target));
    const self = getOffcanvasInstance(element);
    /* istanbul ignore next: must filter */ if (!self) return;
    /* istanbul ignore else */ if (self.options.keyboard && code === keyEscape) {
        self.relatedTarget = null;
        self.hide();
    }
}
 ...
 */ function hideOffcanvasComplete(self, callback) {
    ...
    /* istanbul ignore else */ if (visibleTrigger) focus(visibleTrigger);
    removeOverlay(element);
...
// ====================
/** Returns a new `Offcanvas` instance. */ class Offcanvas extends BaseComponent {
    /**
   ...
   */ get defaults() {
    ...
    /* eslint-enable */ // OFFCANVAS PUBLIC METHODS
    // ========================
    /** Shows or hides the offcanvas from the user. */ toggle() {
        const self = this;
        if (hasClass(self.element, showClass)) self.hide();
        else self.show();
    }
    /** Shows the offcanvas to the user. */ show() {
        const self = this;
        const { element , options , container , relatedTarget ,  } = self;
        let overlayDelay = 0;
        if (hasClass(element, showClass)) return;
        ...
        if (currentOpen && currentOpen !== element) {
            ...
            const that1 = this1 || /* istanbul ignore next */ getInstance(currentOpen, "Modal");
            that1.hide();
        }
        if (options.backdrop) {
            if (!container.contains(overlay)) appendOverlay(container, true);
   ...
   */ hide(callback) {
        ...
        addClass(element, offcanvasTogglingClass);
        removeClass(element, showClass);
        if (!callback) emulateTransitionEnd(element, ()=>beforeOffcanvasHide(self, callback));
        else beforeOffcanvasHide(self, callback);
    }
    /** Removes the `Offcanvas` from the target element. */ dispose() {
        const self = this;
...
/** @type {string} */ const popoverString = "popover";
/** @type {string} */ const popoverComponent = "Popover";
/** @type {string} */ const tooltipString = "tooltip";
/**
 ...
 * @param {any} element the target element
 * @returns {boolean} the query result
 */ const isMedia = (element)=>element && element.nodeType === 1 && [
        "SVG",
        "Image",
        "Video"
 ...
 */ function getRectRelativeToOffsetParent(element, offsetParent, scroll) {
    ...
    /* istanbul ignore next */ if (isParentAnElement) {
...
/** @type {Record<string, string>} */ const tipClassPositions = {
    top: "top",
    bottom: "bottom",
    left: "start",
    right: "end"
 ...
 */ function styleTip(self, e) {
    ...
    const { element , tooltip , options , arrow , offsetParent ,  } = self;
    const tipPositions = {
        ...tipClassPositions
    };
    const RTL = isRTL(element);
    ...
    const { left: parentLeft , right: parentRight , top: parentTop ,  } = getBoundingClientRect(container, true);
    ...
    const { width: elemWidth , height: elemHeight , left: elemRectLeft , right: elemRectRight , top: elemRectTop ,  } = getBoundingClientRect(element, true);
    ...
    if (!tooltip.className.includes(placement)) tooltip.className = tooltip.className.replace(tipClasses, tipPositions[placement]);
    // compute tooltip / popover coordinates
    /* istanbul ignore else */ if (horizontal.includes(placement)) {
        if (placement === "left") leftPosition = x - tipWidth - (isPopover ? arrowWidth : 0);
        else leftPosition = x + elemWidth + (isPopover ? arrowWidth : 0);
        ...
        if (e && isMedia(element)) {
            ...
            if (placement === "top") topPosition = eY - tipHeight - arrowWidth;
            else topPosition = eY + arrowWidth;
            ...
            if (placement === "top") topPosition = y - tipHeight - (isPopover ? arrowHeight : 0);
            else topPosition = y + elemHeight + (isPopover ? arrowHeight : 0);
            // adjust left | right and also the arrow
            if (leftExceed) {
                leftPosition = 0;
    ...
    /* istanbul ignore else */ if (isHTMLElement(arrow)) {
        if (arrowTop !== undefined) arrow.style.top = `${arrowTop}px`;
        if (arrowLeft !== undefined) arrow.style.left = `${arrowLeft}px`;
        else if (arrowRight !== undefined) arrow.style.right = `${arrowRight}px`;
    }
}
const tooltipDefaults = {
    /** @type {string} */ template: getTipTemplate(tooltipString),
    /** @type {string?} */ title: null,
    /** @type {string?} */ customClass: null,
    /** @type {string} */ trigger: "hover focus",
    /** @type {string?} */ placement: "top",
    /** @type {((c:string)=>string)?} */ sanitizeFn: null,
    /** @type {boolean} */ animation: true,
    /** @type {number} */ delay: 200,
 ...
 * @type {boolean}
 */ const isApple = userAgentData ? userAgentData.brands.some((x)=>appleBrands.test(x.brand)) : /* istanbul ignore next */ appleBrands.test(userAgent);
/**
 * Global namespace for `data-bs-title` attribute.
 */ const dataOriginalTitle = "data-original-title";
/** @type {string} */ const tooltipComponent = "Tooltip";
/**
 ...
 */ function setHtml(element, content, sanitizeFn) {
    /* istanbul ignore next */ if (!isHTMLElement(element) || isString(content) && !content.length) return;
    /* istanbul ignore else */ if (isString(content)) {
        ...
        element.append(...[
            ...tempDocument.body.childNodes
        ]);
    } else if (isHTMLElement(content)) element.append(content);
    else if (isNodeList(content) || isArray(content) && content.every(isNode)) element.append(...[
        ...content
    ]);
}
/**
 ...
 */ function createTip(self) {
    ...
    const { animation , customClass , sanitizeFn , placement , dismissible , title , content , template , btnClose ,  } = options;
    ...
    // load template
    /** @type {HTMLElement?} */ let tooltipTemplate;
    ...
    const { arrow  } = self;
    if (isHTMLElement(title)) titleParts = [
        ...
        titleParts = [
            ...tempTitle.childNodes
        ];
    }
    if (isHTMLElement(content)) contentParts = [
        ...
        contentParts = [
            ...tempContent.childNodes
        ];
    }
    // set dismissible button
    if (dismissible) {
        if (title) {
            if (isHTMLElement(btnClose)) titleParts = [
                ...titleParts,
                btnClose.cloneNode(true)
            ];
            else {
            ...
            /* istanbul ignore else */ if (tooltipHeader) tooltipHeader.remove();
            if (isHTMLElement(btnClose)) contentParts = [
                ...contentParts,
                btnClose.cloneNode(true)
            ];
            else {
                const tempBtn1 = createElement("div");
                setHtml(tempBtn1, btnClose, sanitizeFn);
                contentParts = [
                    ...contentParts,
                    tempBtn1.firstChild
                ];
    ...
    /* istanbul ignore else */ if (!isTooltip) {
        /* istanbul ignore else */ if (title && tooltipHeader) setHtml(tooltipHeader, titleParts, sanitizeFn);
        /* istanbul ignore else */ if (content && tooltipBody) setHtml(tooltipBody, contentParts, sanitizeFn);
        // set btn
    ...
    /* istanbul ignore else */ if (!hasClass(tooltip, tipString)) addClass(tooltip, tipString);
    /* istanbul ignore else */ if (animation && !hasClass(tooltip, fadeClass)) addClass(tooltip, fadeClass);
    /* istanbul ignore else */ if (customClass && !hasClass(tooltip, customClass)) addClass(tooltip, customClass);
    /* istanbul ignore else */ if (!hasClass(tooltip, placementClass)) addClass(tooltip, placementClass);
}
/**
 ...
 */ function disposeTooltipComplete(self, callback) {
    ...
    /* istanbul ignore else */ if (hasAttribute(element, dataOriginalTitle) && self.name === tooltipComponent) toggleTooltipTitle(self);
    /* istanbul ignore else */ if (callback) callback();
}
/**
 ...
 */ function toggleTooltipAction(self, add) {
    ...
    /* istanbul ignore else */ if (!isMedia(element)) [
        scrollEvent,
        resizeEvent
    ].forEach((ev)=>{
 ...
 */ function toggleTooltipHandlers(self, add) {
    ...
    self.enabled = !!add;
    /** @type {string[]} */ const triggerOptions = trigger.split(" ");
    const elemIsMedia = isMedia(element);
    if (elemIsMedia) action(element, mousemoveEvent, self.update, passiveHandler);
    triggerOptions.forEach((tr)=>{
        /* istanbul ignore else */ if (elemIsMedia || tr === mousehoverEvent) {
            action(element, mousedownEvent, self.show);
            action(element, mouseenterEvent, self.show);
            /* istanbul ignore else */ if (dismissible && btn) action(btn, mouseclickEvent, self.hide);
            else {
                action(element, mouseleaveEvent, self.hide);
                action(getDocument(element), touchstartEvent, self.handleTouch, passiveHandler);
            }
        } else if (tr === mouseclickEvent) action(element, tr, !dismissible ? self.toggle : self.show);
        else if (tr === focusEvent) {
            action(element, focusinEvent, self.show);
            /* istanbul ignore else */ if (!dismissible) action(element, focusoutEvent, self.hide);
            /* istanbul ignore else */ if (isApple) action(element, mouseclickEvent, ()=>focus(element));
        }
    });
}
/**
...
// TOOLTIP DEFINITION
// ==================
/** Creates a new `Tooltip` instance. */ class Tooltip extends BaseComponent {
    /**
   ...
   */ constructor(target, config){
        ...
        /* istanbul ignore next: this is to set Popover too */ getTooltipInstance = (elem)=>getInstance(elem, tipComponent);
        // additional properties
        /** @type {any} */ self.tooltip = {};
        if (!isTooltip) /** @type {any?} */ self.btn = null;
        /** @type {any} */ self.arrow = {};
        /** @type {any} */ self.offsetParent = {};
        /** @type {boolean} */ self.enabled = true;
        /** @type {string} Set unique ID for `aria-describedby`. */ self.id = `${tipString}-${getUID(element, tipString)}`;
        // instance options
        const { options  } = self;
        // invalidate
        if (!options.title && isTooltip || !isTooltip && !options.content) // throw Error(`${this.name} Error: target has no content set.`);
        return;
        ...
        self.options.container = !container || container && [
            "static",
            "relative"
        ].includes(getElementStyle(container, "position")) ? idealContainer : /* istanbul ignore next */ container || getDocumentBody(element);
        ...
        /* istanbul ignore else */ if (hasAttribute(element, titleAttr) && isTooltip) toggleTooltipTitle(self, options.title);
        // create tooltip here
        createTip(self);
        // attach events
   ...
   */ get defaults() {
    ...
    /* eslint-enable */ // TOOLTIP PUBLIC METHODS
    // ======================
    /**
   * Shows the tooltip.
   *
   ...
   */ show(e) {
        const self = this;
        const { options , tooltip , element , id ,  } = self;
        const { container , animation  } = options;
        const outTimer = Timer.get(element, "out");
        Timer.clear(element, "out");
        if (tooltip && !outTimer && !isVisibleTip(tooltip, container)) Timer.set(element, ()=>{
            ...
            /* istanbul ignore else */ if (!hasClass(tooltip, showClass)) addClass(tooltip, showClass);
            /* istanbul ignore else */ if (animation) emulateTransitionEnd(tooltip, ()=>tooltipShownAction(self));
   ...
   */ hide(callback) {
        ...
        Timer.clear(element, "in");
        /* istanbul ignore else */ if (tooltip && isVisibleTip(tooltip, container)) Timer.set(element, ()=>{
            const hideTooltipEvent = OriginalEvent(`hide.bs.${toLowerCase(self.name)}`);
            dispatchEvent(element, hideTooltipEvent);
            if (hideTooltipEvent.defaultPrevented) return;
            removeClass(tooltip, showClass);
            toggleTooltipOpenHandlers(self);
            /* istanbul ignore else */ if (animation) emulateTransitionEnd(tooltip, ()=>tooltipHiddenAction(self, callback));
   ...
   */ toggle(e) {
    ...
    /** Enables the tooltip. */ enable() {
        const self = this;
        const { enabled  } = self;
        /* istanbul ignore else */ if (!enabled) {
            toggleTooltipHandlers(self, true);
            self.enabled = !enabled;
        }
    }
    /** Disables the tooltip. */ disable() {
        const self = this;
        const { tooltip , options , enabled ,  } = self;
        const { animation , container  } = options;
        /* istanbul ignore else */ if (enabled) {
            if (isVisibleTip(tooltip, container) && animation) self.hide(()=>toggleTooltipHandlers(self));
            else toggleTooltipHandlers(self);
            self.enabled = !enabled;
        }
    }
    /** Toggles the `disabled` property. */ toggleEnabled() {
        const self = this;
   ...
   */ handleTouch({ target  }) {
        ...
        /* istanbul ignore next */ if (tooltip.contains(target) || target === element || target && element.contains(target)) ;
        else this.hide();
    }
    /** Removes the `Tooltip` from the target element. */ dispose() {
        const self = this;
        const { tooltip , options  } = self;
...
const popoverDefaults = {
    ...tooltipDefaults,
    /** @type {string} */ template: getTipTemplate(popoverString),
    /** @type {string} */ btnClose: '<button class="btn-close" aria-label="Close"></button>',
    /** @type {boolean} */ dismissible: false,
    /** @type {string?} */ content: null
};
// POPOVER DEFINITION
// ==================
/** Returns a new `Popover` instance. */ class Popover extends Tooltip {
   ...
   */ get defaults() {
    ...
    /* eslint-enable */ /* extend original `show()` */ show() {
        super.show();
        // btn only exists within dismissible popover
        const { options , btn  } = this;
        /* istanbul ignore else */ if (options.dismissible && btn) setTimeout(()=>focus(btn), 17);
    }
}
/**
...
/** @type {string} */ const scrollspyString = "scrollspy";
/** @type {string} */ const scrollspyComponent = "ScrollSpy";
 ...
 */ function updateSpyTargets(self) {
    ...
    /* istanbul ignore else */ if (links && (itemsLength !== links.length || scrollHEIGHT !== scrollHeight)) {
        let href;
        let targetItem;
        let rect;
        // reset arrays & update
        ...
        [
            ...
            targetItem = href && href.charAt(0) === "#" && href.slice(-1) !== "#" && querySelector(href, getDocument(element));
            if (targetItem) {
 ...
 */ function getScrollHeight(scrollTarget) {
    return isHTMLElement(scrollTarget) ? scrollTarget.scrollHeight : getDocumentElement(scrollTarget).scrollHeight;
}
/**
 ...
 */ function getOffsetHeight({ element , scrollTarget  }) {
    return isWindow(scrollTarget) ? scrollTarget.innerHeight : getBoundingClientRect(element).height;
}
/**
 ...
 */ function activate(self, item) {
    ...
    parents.forEach((menuItem)=>{
        /** @type {HTMLElement?} */ const parentLink = menuItem.previousElementSibling;
        /* istanbul ignore else */ if (parentLink && !hasClass(parentLink, activeClass)) addClass(parentLink, activeClass);
    });
    // dispatch
...
// ====================
/** Returns a new `ScrollSpy` instance. */ class ScrollSpy extends BaseComponent {
    /**
   * @param {HTMLElement | string} target the target element
   ...
   */ constructor(target, config){
        ...
        const { element , options  } = self;
        // additional properties
        /** @type {HTMLElement?} */ self.target = querySelector(options.target, getDocument(element));
        // invalidate
        if (!self.target) return;
        // set initial state
        /** @type {HTMLElement | Window} */ self.scrollTarget = element.clientHeight < element.scrollHeight ? element : getWindow(element);
        /** @type {number} */ self.scrollTop = 0;
        /** @type {number} */ self.maxScroll = 0;
        /** @type {number} */ self.scrollHeight = 0;
        /** @type {HTMLElement?} */ self.activeItem = null;
        /** @type {HTMLElement[]} */ self.items = [];
        /** @type {number} */ self.itemsLength = 0;
        /** @type {number[]} */ self.offsets = [];
        // bind events
   ...
   */ get defaults() {
    ...
    /* eslint-enable */ // SCROLLSPY PUBLIC METHODS
    // ========================
    /** Updates all items. */ refresh() {
        const self = this;
        const { target  } = self;
        // check if target is visible and invalidate
        /* istanbul ignore next */ if (target.offsetHeight === 0) return;
        updateSpyTargets(self);
        const { scrollTop , maxScroll , itemsLength , items , activeItem ,  } = self;
        if (scrollTop >= maxScroll) {
            const newActiveItem = items[itemsLength - 1];
            /* istanbul ignore else */ if (activeItem !== newActiveItem) activate(self, newActiveItem);
            return;
        }
        const { offsets  } = self;
        ...
        items.forEach((item, i)=>{
            if (activeItem !== item && scrollTop >= offsets[i] && (typeof offsets[i + 1] === "undefined" || scrollTop < offsets[i + 1])) activate(self, item);
        });
    }
    /** Removes `ScrollSpy` from the target element. */ dispose() {
...
/** @type {string} */ const tabString = "tab";
/** @type {string} */ const tabComponent = "Tab";
 ...
 */ function triggerTabEnd(self) {
    ...
    /* istanbul ignore else */ if (tabContent && hasClass(tabContent, collapsingClass)) {
        tabContent.style.height = "";
        removeClass(tabContent, collapsingClass);
    }
    /* istanbul ignore else */ if (nav) Timer.clear(nav);
}
/**
 ...
 */ function triggerTabShow(self) {
    ...
    /* istanbul ignore else */ if (tabContent && hasClass(nextContent, fadeClass)) {
        const { currentHeight , nextHeight  } = tabPrivate.get(element);
        if (currentHeight === nextHeight) triggerTabEnd(self);
        else // enables height animation
        setTimeout(()=>{
 ...
 */ function triggerTabHide(self) {
    ...
    if (nextContent && hasClass(nextContent, fadeClass)) setTimeout(()=>{
 ...
 */ function getActiveTab(self) {
    ...
    /* istanbul ignore else */ if (activeTabs.length === 1 && !dropdownMenuClasses.some((c)=>hasClass(activeTabs[0].parentElement, c))) [tab] = activeTabs;
...
// TAB DEFINITION
// ==============
/** Creates a new `Tab` instance. */ class Tab extends BaseComponent {
    /**
   * @param {HTMLElement | string} target the target element
   */ constructor(target){
        super(target);
        ...
        /** @type {HTMLElement?} */ self.nav = nav;
        /** @type {HTMLElement} */ self.content = content;
        /** @type {HTMLElement?} */ self.tabContent = container;
        // event targets
        /** @type {HTMLElement?} */ self.dropdown = getParentDropdown(element);
        ...
        if (nav && !tab) {
            ...
            /* istanbul ignore else */ if (firstTabContent) {
   ...
   */ get name() {
    ...
    /* eslint-enable */ // TAB PUBLIC METHODS
    // ==================
    /** Shows the tab to the user. */ show() {
        const self = this;
        const { element , content: nextContent , nav , dropdown ,  } = self;
        /* istanbul ignore else */ if (!(nav && Timer.get(nav)) && !hasClass(element, activeClass)) {
            const { tab , content  } = getActiveTab(self);
            /* istanbul ignore else */ if (nav) tabPrivate.set(nav, {
                tab,
                content
            });
            ...
            if (activeDropdown && hasClass(activeDropdown, activeClass)) removeClass(activeDropdown, activeClass);
            /* istanbul ignore else */ if (nav) {
                const toggleTab = ()=>{
                    removeClass(tab, activeClass);
                    setAttribute(tab, ariaSelected, "false");
                    if (dropdown && !hasClass(dropdown, activeClass)) addClass(dropdown, activeClass);
                };
                if (hasClass(content, fadeClass) || hasClass(nextContent, fadeClass)) Timer.set(nav, toggleTab, 1);
                else toggleTab();
            }
            removeClass(content, showClass);
            if (hasClass(content, fadeClass)) emulateTransitionEnd(content, ()=>triggerTabHide(self));
            else triggerTabHide(self);
        }
    }
    /** Removes the `Tab` component from the target element. */ dispose() {
...
/** @type {string} */ const toastString = "toast";
/** @type {string} */ const toastComponent = "Toast";
...
/** @deprecated */ const hideClass = "hide";
const toastDefaults = {
    animation: true,
    autohide: true,
    delay: 5000
 ...
 */ function showToastComplete(self) {
    ...
    /* istanbul ignore else */ if (options.autohide) Timer.set(element, ()=>self.hide(), options.delay, toastString);
}
/**
 ...
 */ function toggleToastHandlers(self, add) {
    ...
    /* istanbul ignore else */ if (dismiss) action(dismiss, mouseclickEvent, self.hide);
    /* istanbul ignore else */ if (options.autohide) [
        focusinEvent,
        focusoutEvent,
        mouseenterEvent,
        mouseleaveEvent
    ].forEach((e)=>action(element, e, interactiveToastHandler));
    /* istanbul ignore else */ if (triggers.length) triggers.forEach((btn)=>action(btn, mouseclickEvent, toastClickHandler));
}
 ...
 */ function interactiveToastHandler(e) {
    ...
    /* istanbul ignore next: a solid filter is required */ if (!self || element === relatedTarget || element.contains(relatedTarget)) return;
    if ([
        mouseenterEvent,
        focusinEvent
...
// TOAST DEFINITION
// ================
/** Creates a new `Toast` instance. */ class Toast extends BaseComponent {
    /**
   ...
   */ constructor(target, config){
        ...
        // dismiss button
        /** @type {HTMLElement?} */ self.dismiss = querySelector(toastDismissSelector, element);
   ...
   */ get isShown() {
    ...
    // ====================
    /** Shows the toast. */ show() {
        const self = this;
        const { element , isShown  } = self;
        /* istanbul ignore else */ if (element && !isShown) {
            dispatchEvent(element, showToastEvent);
            if (showToastEvent.defaultPrevented) return;
            showToast(self);
        }
    }
    /** Hides the toast. */ hide() {
        const self = this;
        const { element , isShown  } = self;
        /* istanbul ignore else */ if (element && isShown) {
    ...
    /** Removes the `Toast` component from the target element. */ dispose() {
        const self = this;
        const { element , isShown  } = self;
        /* istanbul ignore else */ if (isShown) removeClass(element, showClass);
...
/** @type {Record<string, any>} */ const componentsList = {
    Alert,
    Button,
    Carousel,
    Collapse,
...
else addListener(document, "DOMContentLoaded", ()=>initCallback(), {
    once: true
});
const BSN = {
    Alert,
...
},{}]},["kFn5I","lDHsX"], "lDHsX", "parcelRequirea42f")

//# sourceMappingURL=06-promises.11073073.js.map
